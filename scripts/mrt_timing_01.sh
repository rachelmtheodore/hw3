...
# For example, "scripts/mrt_timing_01.sh 10" will specify running the loop 10 times.
...
do
	...
	-stim_labels A B C \
	-num_reps 20 \
	-prefix times/replicate/stimt_${iteration} \
	-stim_dur 1 \
	-seed ${iteration}
	3dDeconvolve \
	-nodata 300 1 -polort 1 \
	-num_stimts 3 \
	-stim_times 1 times/replicate/stimt_${iteration}_01_A.1D "GAM" -stim_label 1 "A" \
	-stim_times 2 times/replicate/stimt_${iteration}_02_B.1D "GAM" -stim_label 2 "B" \
	-stim_times 3 times/replicate/stimt_${iteration}_03_C.1D "GAM" -stim_label 3 "C" \
	-gltsym "SYM: A -B" -gltsym "SYM: A -C" \
	> times/replicate/out_${iteration}.txt
	efficiency=`scripts/efficiency_parser.py times/replicate/out_${iteration}.txt`
	echo "$efficiency $iteration" >> results/results.replicate.txt
done
...
# (sorted) file.
cat results/results.replicate.txt | sort -n -k 1 > results/results.replicate.sort.txt
head results/results.replicate.sort.txt
