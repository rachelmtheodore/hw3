...
do
	...
	-stim_labels A B C \
	-num_reps 20 \
	-prefix mrt_times/stimt_${iteration} \
	-stim_dur 2 \
	-min_rest 1 \
	-max_rest 5 \
	-seed ${iteration}
	...
	-num_stimts 3 \
	-stim_times 1 mrt_times/stimt_${iteration}_01_A.1D "GAM" -stim_label 1 "A" \
	-stim_times 2 mrt_times/stimt_${iteration}_02_B.1D "GAM" -stim_label 2 "B" \
	-stim_times 3 mrt_times/stimt_${iteration}_03_C.1D "GAM" -stim_label 3 "C" \
	-gltsym "SYM: A -B" -gltsym "SYM: A -C" \
	> mrt_times/out_${iteration}.txt
	efficiency=`scripts/efficiency_parser.py mrt_times/out_${iteration}.txt`
	echo "$efficiency $iteration" >> results/results.mrt.txt
done
...
# (sorted) file.
cat results/results.mrt.txt | sort -n -k 1 > results/results.mrt.sort.txt
head results/results.mrt.sort.txt
